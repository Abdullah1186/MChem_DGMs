...
try:
    ...
    pass
import build_oe62_dataset
from configs.datasets_config import oe62_with_h
import copy
import utils
import argparse
import wandb
...
parser.add_argument('--dataset', type=str, default='oe62',
                    help='dataset name')
...
data_file = '/mnt/scratch2/users/afekaiki/GeoLDM_oe62/GeoLDM/data/oe62/geom_oe62_1.npy'

if args.remove_h:
    raise NotImplementedError()
else:
    dataset_info = oe62_with_h

args.cuda = not args.no_cuda and torch.cuda.is_available()
device = torch.device("cuda" if args.cuda else "cpu")
dtype = torch.float32

split_data = build_oe62_dataset.load_split_data(data_file, val_proportion=0.1, test_proportion=0.1, filter_size=args.filter_molecule_size)
transform = build_oe62_dataset.GeomDrugsTransform(dataset_info, args.include_charges, device, args.sequential)
dataloaders = {}
for key, data_list in zip(['train', 'val', 'test'], split_data):
    dataset = build_oe62_dataset.GeomDrugsDataset(data_list, transform=transform)
    shuffle = (key == 'train') and not args.sequential

    # Sequential dataloading disabled for now.
    dataloaders[key] = build_oe62_dataset.GeomDrugsDataLoader(
...
kwargs = {'entity': args.wandb_usr, 'name': args.exp_name, 'project': 'e3_diffusion_oe62_2', 'config': args,
