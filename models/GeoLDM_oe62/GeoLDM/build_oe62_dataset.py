...
def extract_conformers(args):
    oe62_file = os.path.join(args.data_dir, args.data_file)
    save_file = f"geom_oe62_{'no_h_' if args.remove_h else ''}{args.conformations}"
    smiles_list_file = 'geom_oe62_smiles.txt'
    number_atoms_file = f"geom_oe62_n_{'no_h_' if args.remove_h else ''}{args.conformations}"

    unpacker = msgpack.Unpacker(open(oe62_file, "rb"))

    all_smiles = []
    all_number_atoms = []
    dataset_conformers = []
    mol_id = 0
    for i, oe62_1k in enumerate(unpacker):
        print(f"Unpacking file {i}...")
        for smiles, all_info in oe62_1k.items():
...
def load_split_data(conformation_file, val_proportion=0.1, test_proportion=0.1,
    ...
    #         'reproducibility.')
    # assert not os.path.exists(os.path.join(base_path, 'oe62_permutation.npy'))
    # np.save(os.path.join(base_path, 'oe62_permutation.npy'), perm)
    # del perm

    perm = np.load(os.path.join(base_path, 'oe62_permutation.npy'))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_dir", type=str, default='/mnt/scratch2/users/afekaiki/GeoLDM_oe62/GeoLDM/data/oe62')
    parser.add_argument("--data_file", type=str, default="oe62_crude.msgpack")
