...
def retrieve_dataloaders(cfg):
    if 'qm9' in cfg.dataset:
        ...
        split_data = build_oe62_dataset.load_split_data(data_file,
                                                        val_proportion=0.1,
                                                        test_proportion=0.1,
                                                        filter_size=cfg.filter_molecule_size)
        transform = build_oe62_dataset.GeomDrugsTransform(dataset_info,
                                                          cfg.include_charges,
                                                          cfg.device,
                                                          cfg.sequential)
        dataloaders = {}
        for key, data_list in zip(['train', 'val', 'test'], split_data):
            dataset = build_oe62_dataset.GeomDrugsDataset(data_list,
                                                          transform=transform)
