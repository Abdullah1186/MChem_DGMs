...
def get_parser():
    ...
    main_parser.add_argument('--valence',
                             default = [1,1, 5,3, 6,4, 7,3, 8,2, 9,1, 13,3, 14,4, 15,3, 16,2, 17,1, 33,3, 34,2, 35,1, 53,1, 80,2, 83,3], type=int,
                             nargs='+',
...
#                    res[i]['_atomic_numbers'] =  list(res2[i]['_atomic_numbers'])
    ...
    stat_heads = ['n_atoms', 'id', 'valid', 'duplicating', 'n_duplicates',
                  'known', 'equals', 'C', 'N', 'O', 'F', 'H','B','Si','P','S','Cl','Se','Br','I', 'H1C', 'H1N',
    ...
    for n_atoms in res:
        ...
        n_of_types = [np.sum(all_numbers == i, axis=1) for i in [1, 5, 6, 7, 8, 9, 14, 15, 16, 17, 34, 35, 53]]
        stats_new = np.stack(
